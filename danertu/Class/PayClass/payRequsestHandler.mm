...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPayWithDict:(NSMutableDictionary *)orderInfoDic
{
    
    //订单标题，展示给用户
    NSString *order_name    = [orderInfoDic objectForKey:@"subject"];
    //订单金额,单位（分）
    NSString *priceStr = [NSString stringWithFormat:@"%@",[orderInfoDic objectForKey:@"price"]];
    NSInteger price = [priceStr floatValue] * 100;
    NSString *order_price = [NSString stringWithFormat:@"%ld",(long)price];
    if(isTestPayMoney){
        order_price = @"1";
    }
    //================================
    ...
    NSString *orderno   = [orderInfoDic objectForKey:@"tradeNO"]; //订单ID（由商家自行制定）
    NSMutableDictionary *packageParams = [NSMutableDictionary dictionary];
    
    [packageParams setObject: appid             forKey:@"appid"];       //开放平台appid
    [packageParams setObject: mchid             forKey:@"mch_id"];      //商户号
    [packageParams setObject: [Tools getDeviceUUid]        forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: order_name        forKey:@"body"];        //订单描述，展示给用户
    [packageParams setObject: WXNOTIFY_URL        forKey:@"notify_url"];  //支付结果异步通知
