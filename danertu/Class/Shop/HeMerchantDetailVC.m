...
@end
@implementation HeMerchantDetailVC
...
{
    if (!self.shopIdFrom) {
        self.shopIdFrom = SHOPID;
    }
    [self showHudInView:self.view hint:@"努力加载中..."];
    [self performSelector:@selector(dismissTip) withObject:nil afterDelay:20];
...
- (void)requestShopInfo
{
    ...
    if (!shopId) {
        shopId = shopIdFrom;//没有modle,只传来了shopId
    }
    NSDictionary *paramsDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"0041",@"apiid",shopId,@"shopid",[[[LocationSingleton sharedInstance] pos] objectForKey:@"lat"],@"la",[[[LocationSingleton sharedInstance] pos] objectForKey:@"lot"],@"lt", nil];
    [[AFOSCClient sharedClient] postPath:nil parameters:paramsDict success:^(AFHTTPRequestOperation *operation, id responseObject){
        ...
        shopInfoDic = [[[[jsonData objectForKey:@"shopdetails"] objectForKey:@"shopbean"][0] mutableCopy] mutableCopy];
        //初始化model
        if (!modle) {
            [shopInfoDic setObject:shopIdFrom forKey:@"shopId"];
            [self initModleData];
        }else{
...
-(void)initModleData
{
    ...
    modle.id = [shopInfoDic objectForKey:@"shopId"];
...
-(void)tapTabbar:(UIButton *)btn{
    ...
    switch (tag) {
        ...
        case 5:{
            ...
            NSDictionary *shareInfo = [NSDictionary dictionaryWithObjectsAndKeys:modle.s,@"shareTitle",shopIdFrom,@"shareId",@"s",@"shareType",nil];
    ...
    if(!requestOperation.error){
        ...
        if (jsonData) {
            ...
            if(temp.count>0){
                ...
                //            //------没有商品数据时显示
                //            if (!noDataView) {
                //                noDataView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 90)];
                //                [clientGoodsView addSubview:noDataView];
                //                UIImageView *noDataImg = [[UIImageView alloc] initWithFrame:CGRectMake(125, 0, 70, 70)];
                //                [noDataView addSubview: noDataImg];
                //                [noDataImg setImage:[UIImage imageNamed:@"noData"]];
                //                UILabel *noDataText = [[UILabel alloc] initWithFrame:CGRectMake(100, 70, 120, 20)];
                //                [noDataView addSubview:noDataText];
                //                noDataText.textAlignment = NSTextAlignmentCenter;
                //                noDataText.font = TEXTFONT;
                //                noDataText.textColor = [UIColor grayColor];
                //                noDataText.text = @"暂无线下产品";
                //            }
            }
...
-(void)createClientWoodView{
    ...
    CGRect frame1 = CGRectMake(0, 25, 320, lineNum*height);
...
-(void)createPageView:(NSString *)pageName showHeader:(BOOL) isShow{
    //-------新模板,页面构造------,头部使用html页面--------
    CGRect frame = CGRectMake(0, addStatusBarHeight, self.view.frame.size.width, self.view.frame.size.height-addStatusBarHeight);
    if (isShow) {
        frame = CGRectMake(0, addStatusBarHeight+TOPNAVIHEIGHT, self.view.frame.size.width, CONTENTHEIGHT+49);
    }
...
-(void)createIosView{
    ...
    //scrollView.scrollEnabled = NO;//-----------禁止滚动--------
    NSLog(@"faniobejoifjwoef---%f",self.view.frame.size.height);
    
    //加盟商自己产品
    UIView *clientItem = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 160)];
    clientItem.backgroundColor = [UIColor whiteColor];
    clientItem.userInteractionEnabled = YES;
    [scrollView addSubview:clientItem];
    
    bannerProductV = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, 320, 120)];
    [clientItem addSubview:bannerProductV];
    bannerProductV.backgroundColor = [UIColor whiteColor];
    
    //图片
    UIImageView *clientImg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, 120)];
    ...
    if ([[shopInfoDic objectForKey:@"shopshowproduct"] length]>5) {
        ...
        if (productInfoArr.count>4) {
            ...
            NSLog(@"kopergkopembjhoth-----%@--%@",imgUrl,productInfoArr);
            
            //-----mask蒙版-----上边一个名词,一个按钮
            UIView *maskView_ = [[UIView alloc] initWithFrame:CGRectMake(320, 0, 320, 120)];
            ...
            UILabel *border =[[UILabel alloc] initWithFrame:CGRectMake(0, 119, 320, 1)];
    ...
    //-----mask蒙版-----上边一个名词,一个按钮
    UIView *maskView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 120)];
    ...
    UIView *addressDistanceView = [[UILabel alloc] initWithFrame:CGRectMake(0, bannerProductV.frame.size.height, 320, 40)];
    ...
    if (modle.juli.length>0) {
        ...
        distanceLb.text = [NSString stringWithFormat:@"%0.2f km",distance];
        
    }
    //-----------名酒世界-----吼一吼---------
    UIView *adsView = [[UIView alloc] initWithFrame:CGRectMake(0,clientItem.frame.size.height,320,130)];
    ...
    //-------本店先下产品-----
    UIView * clientAreaView = [[UIView alloc] initWithFrame:CGRectMake(0, clientItem.frame.size.height+adsView.frame.size.height, 320, 225)];
    [scrollView addSubview:clientAreaView];
    clientAreaView.backgroundColor = [UIColor whiteColor];
    
    //-----title---
    UILabel *titleLb = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 320, 25)];
    ...
    //-----线下产品展示-----创建view-----
    clientGoodsView = [[UIView alloc] initWithFrame:CGRectMake(0, 25, 320, 200)];
    [clientAreaView addSubview:clientGoodsView];
    [self createClientWoodView];
    clientAreaView.frame = CGRectMake(0, clientItem.frame.size.height+adsView.frame.size.height, 320, 25+clientGoodsView.frame.size.height);
    
    //
    UIView *navigationTelView = [[UIView alloc] initWithFrame:CGRectMake(0, clientItem.frame.size.height+adsView.frame.size.height+clientAreaView.frame.size.height, 320, 35)];
    ...
    UILabel *border = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 320, 1)];
    ...
    if (parameterCount>1) {
        ...
        if (memLoginId) {
            ...
            AFHTTPRequestOperation * operationItem = [httpClient HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operationItem, id responseObject) {
                [Waiting dismiss];
                NSString *source = [[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding];
                NSLog(@"jgorejggjieogjieoojroinobrt-----%@\n%@\n%@",sourceStr,source,params);
...
-(void)timerFired{
    //-----滑动----
    if(bannerProductV.contentOffset.x==320){
        ...
        [UIView animateWithDuration:2.0 animations:^(void){
            [bannerProductV setContentOffset:CGPointMake(320, 0) animated:YES];
        }];
    }
}
//------广告点击跳转----
...
-(void)onClickQR{
    ...
    NSString *shareTitleStr = @"单耳兔美食网";    //分享的主标题
    ...
    //    NSDictionary *shareInfo = [NSDictionary dictionaryWithObjectsAndKeys:modle.s,@"shareTitle",modle.m,@"shareId",@"s",@"shareType",nil];
...
{
    NSInteger cellCount = arrays.count;
    return cellCount;
}
...
-(void)goNavigation{
    ...
    if (iOS8) {
        ...
        UIAlertAction *defaultLocateAction = [UIAlertAction actionWithTitle:@"使用系统自带地图导航" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            //取消
            double endLat = [modle.la doubleValue] /1000000;
            double endLot = [modle.lt doubleValue] /1000000;
...
- (void)availableMapsApps {
    ...
    if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"baidumap://map/"]]){
        NSString *urlString = [NSString stringWithFormat:@"baidumap://map/direction?origin=latlng:%f,%f|name:我的位置&destination=latlng:%f,%f|name:%@&mode=driving&coord_type=gcj02",
                               startCoor.latitude, startCoor.longitude, endCoor.latitude, endCoor.longitude, endText];
...
-(void)dialPhone{
     ...
     */
    NSLog(@"jafoiejiogrhufjeio--%@",shopInfoDic);
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if ([alertView tag] ==12 ) {    // it's the Error alert
        ...
        NSLog(@"----alertview--tag------%ld-------%ld",(long)[alertView tag],(long)buttonIndex);
        if(buttonIndex==1){
...
-(void)nearbyShop{
    GoodFoodController *goodV = [[GoodFoodController alloc] init];
    goodV.shopGps = [NSString stringWithFormat:@"%f,%f",[modle.la doubleValue] /1000000,[modle.lt doubleValue] /1000000];
    goodV.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:goodV animated:YES];
}
...
//-----------内存不足报警------
-(void)dealloc{
    NSLog(@"jorejoijhiotroikoqdoejifrg------3");
}
@end
