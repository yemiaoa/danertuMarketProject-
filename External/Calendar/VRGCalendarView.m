...
-(void)showNextMonth {
    //-----"已签"是以UILabel字符串的形式添加到视图上的,所以要删掉,注意表头年月的显示也是 标签
    //-----注意这个循环,每次数组都会变,所有   i  不要递增
    for (int i=1;i<[[self subviews] count];) {
        UIView *tmp = [[self subviews] objectAtIndex:i];
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSLog(@"bjorejgiojreogjei----%@--%@",formatter,self.currentMonth);
    ...
    //上半部分
    CGRect rectangle = CGRectMake(0,0,self.frame.size.width,kVRGCalendarViewTopBarHeight-20);
    CGContextAddRect(context, rectangle);
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"394147"].CGColor);
    CGContextFillPath(context);
    //下半部分
    CGRect rectangle_ = CGRectMake(0,kVRGCalendarViewTopBarHeight-20,self.frame.size.width,20);
    CGContextAddRect(context, rectangle_);
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"8091a8"].CGColor);
    CGContextFillPath(context);
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 43, kVRGCalendarViewDayWidth+2, 17) withFont:font lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        //--UILabel添加的视图--
        UILabel *signText = [[UILabel alloc]initWithFrame:CGRectMake(targetX-5, targetY-8, 45, 15)];
        [self addSubview:signText];
        signText.text = @"已签";
        signText.backgroundColor = [UIColor clearColor];
        signText.textColor = [UIColor redColor];
        signText.textAlignment = NSTextAlignmentCenter;
        signText.font = TEXTFONTSMALL;
        
        /*
         //------字符串划入视图的方式----
        UIFont *font = [UIFont fontWithName:@"HelveticaNeue" size:12];
        [@"已签" drawInRect:CGRectMake(targetX-5, targetY-8, 45, 15) withFont:font lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
         */
        
        /*
        CGContextAddRect(context, rectangle);

        UIColor *color = (UIColor *)[markedColors objectAtIndex:i];
        //--点击了标记的   日期后状态的改变
