...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
         _emojiArr = [NSArray arrayWithObjects:@"[wx]",@"[tx]",@"[hx]",@"[dy]",@"[ll]",@"[sj]",@"[dk]",@"[jy]",@"[zj]",@"[gg]",@"[kx]",@"[zb]",@"[yw]",@"[dn]",@"[sh]",@"[ku]",@"[fn]",@"[ws]",@"[ok]",@"[xh]",nil];
        _text = @"";
...
- (NSString *)analyzeText_txt_replace_emoji:(NSString *)string
{ 
    [self.richTextRunsArray removeAllObjects];
    [self.richTextRunRectDic removeAllObjects];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (!touched) {
        if (self.delegage && [self.delegage respondsToSelector:@selector(richTextView:touchEndOutOfRun:)]) {
            [_delegage richTextView:self touchEndOutOfRun:nil];
        }
    }
}
...
- (void)setText:(NSString *)text
{
    ...
    _text = text;
    _textAnalyzed_txt_replace_emoji = [self analyzeText_txt_replace_emoji:text];
}
