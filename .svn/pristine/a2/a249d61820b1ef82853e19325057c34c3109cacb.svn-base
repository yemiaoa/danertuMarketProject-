...
+ (void)show{
    ...
    NSInteger imageWide = 50;
    
    _imageView = [[UIImageView alloc] initWithFrame:CGRectMake((MAINSCREEN_WIDTH-imageWide)/2, (MAINSCREEN_HEIGHT-imageWide)/2, imageWide, imageWide)];
    ...
    for (int i = 1; i < 28; i ++) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"loading_det%d",i]];
        [arr addObject:image];
    }
    [_imageView setAnimationImages:arr];
    [_imageView setAnimationDuration:2.0];
