...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        if (![_photo.url hasSuffix:@"gif"]) {
            ...
            if ([fileManager fileExistsAtPath:_photo.url]) {
                photo.image = [UIImage imageWithContentsOfFile:_photo.url];
                [photoView adjustFrame];
            }else {
                [_imageView sd_setImageWithURL:[NSURL URLWithString:_photo.url] placeholderImage:_photo.placeholder options:SDWebImageRetryFailed|SDWebImageLowPriority completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                    photo.image = image;
                    // 调整frame参数
                    [photoView adjustFrame];
                }];
...
- (void)photoStartLoad
{
    ...
    if (_photo.image) {
        ...
        if (![fileManager fileExistsAtPath:_photo.url]) {
            ...
            [_imageView sd_setImageWithURL:[NSURL URLWithString:_photo.url] placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
